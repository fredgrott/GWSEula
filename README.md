...
Library uses the version.code as a key to tie the Eula so that when an update is made it can be
shown again to the user beyond the first time show at the first install of the application.
...
```groovy
compile 'com.github.shareme:GWSEula:{latest-release-number}@aar'
```
